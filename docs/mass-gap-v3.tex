...
\begin{abstract}
We analyze the Hubbard-Stratonovich approach to the mass gap for
the O($N$) model in two dimensions. The HS identity for the
Euclidean quartic requires imaginary coupling (sign problem).
For the NLSM, Kupiainen's trick adds a real mass $m_0^2$ for
free via the constraint, giving decay for each $\sigma$-value.
For the LSM, no such trick exists: the real mass cancels after HS,
and the mass gap must emerge entirely from the oscillatory
$\sigma$-average. We compare the two settings and discuss the
implications for a formal proof at large~$N$.
\end{abstract}
