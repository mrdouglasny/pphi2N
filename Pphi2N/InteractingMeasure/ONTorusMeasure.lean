...
def siteField (φ : Fin N → (Fin d → Fin M) → ℝ) (x : Fin d → Fin M) :
    Fin N → ℝ :=
  fun i => φ i x

/-- |φ(x)|² = Σᵢ (φⁱ(x))² at a single site. -/
def siteNormSq (φ : Fin N → (Fin d → Fin M) → ℝ) (x : Fin d → Fin M) : ℝ :=
  ∑ i : Fin N, (φ i x) ^ 2

/-- |φ(x)|² ≥ 0. -/
theorem siteNormSq_nonneg (φ : Fin N → (Fin d → Fin M) → ℝ) (x : Fin d → Fin M) :
...
def onInteraction (P : ONInteraction) (c a : ℝ) :
    (Fin N → (Fin d → Fin M) → ℝ) → ℝ :=
  fun φ => a ^ d * ∑ x : Fin d → Fin M,
...
def onInteractingMeasure (P : ONInteraction) (c a : ℝ)
    (μ_scalar : Measure ((Fin d → Fin M) → ℝ)) :
    Measure (Fin N → (Fin d → Fin M) → ℝ) :=
